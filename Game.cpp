...
void Game::generateSpritesInMemory() {
    int nbObjects = map->getNthGrid(current_grid)->NB_SPRITES +
                    map->getNthGrid(current_grid + 1)->NB_SPRITES;
    ...
    for (int i = first; i < current_grid + 2; i++) {
        ...
        while (ptr) {
            if (ptr->type == "brick") {
                ...
                this->s_objects->setTextureRect(
                    sf::IntRect(272, 112, TILE_DIMENSION, TILE_DIMENSION));
                this->s_objects->setPosition(
                    ptr->position.x +
    ...
    while (this->window->pollEvent(event)) {
        switch (event.type) {
            ...
            case sf::Event::KeyReleased:
                // std::cout << "Key released:" + std::to_string(event.key.code)
                // << std::endl;
